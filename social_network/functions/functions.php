	...
	if(isset($_POST['sub'])){
		...
		if(strlen($content) > 250){
			...
			if(strlen($upload_image) >= 1 && strlen($content) >= 1){
				...
				if($run){
					echo "<script>alert('Your Post was Uploaded')</script>";
					echo "<script>window.open('home.php', '_self')</script>";
    ...
    if(isset($_GET['post_id'])){
        ...
        if($p_id != $post_id){
            ...
            if(isset($_POST['reply'])){
                ...
                if($comment == ""){
                    echo"<script>alert('Did not enter comment')</script>";
...
function search_user(){
    ...
    while($row_user=mysqli_fetch_array($run_user)){
        ...
        <div class='col-sm-6'>
        <a style='text-decoration:none;cursor:pointer;color:#d46660;' href=user_profile.php?u_id=$user_id'>
